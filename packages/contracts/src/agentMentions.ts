/**
 * Agent Mentions — `@alias(task)` syntax for subagent delegation in the composer.
 *
 * Provides provider-aware alias metadata used by the composer UI and provider runtimes.
 * For Codex aliases, the alias maps to a model swap (e.g. `@codex(...)` runs the inner
 * task on `gpt-5.3-codex`). For Claude aliases, the alias maps to a Claude subagent
 * with preset tools, model, and prompt.
 */
...
/**
 * All available aliases for a provider. When no provider is passed, returns the
 * global union (used by parsers/validators that accept any alias).
 */
...
/** The curated set of aliases the composer surfaces in autocomplete. */
...
/**
 * Resolve an alias to its definition. When a provider is passed, only that
 * provider's aliases are considered; otherwise we scan all providers.
 */
