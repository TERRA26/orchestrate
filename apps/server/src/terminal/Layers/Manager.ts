...
function shouldStripOscSequence(content: string): boolean {
  return (
    ...
    content.startsWith(ORCHESTRATE_TERMINAL_HOOK_OSC_PREFIX)
  );
}
...
function extractOscHookEvent(content: string): TerminalAgentHookEventType | null {
  if (!content.startsWith(ORCHESTRATE_TERMINAL_HOOK_OSC_PREFIX)) {
    return null;
  }
  const eventType = content.slice(ORCHESTRATE_TERMINAL_HOOK_OSC_PREFIX.length).trim();
...
function shouldExcludeTerminalEnvKey(key: string): boolean {
  ...
  if (normalizedKey.startsWith("ORCHESTRATE_")) {
    return true;
  }
...
function cliKindFromRuntimeEnv(
  ...
  return terminalCliKindFromValue(runtimeEnv?.[ORCHESTRATE_TERMINAL_CLI_KIND_ENV_KEY]);
}
