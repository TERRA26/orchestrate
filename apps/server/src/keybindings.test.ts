import { KeybindingCommand, KeybindingRule, KeybindingsConfig } from "@orchestrate/contracts";
    ...
    Layer.provideMerge(
      Layer.fresh(
        ServerConfig.layerTest(process.cwd(), {
          prefix: "orchestrate-keybindings-test-",
        }),
      ),
    ),
  );
...
it.layer(NodeServices.layer)("keybindings", (it) => {
  ...
  it.effect("accepts legacy thread navigation entries from older configs", () =>
    Effect.gen(function* () {
      const { keybindingsConfigPath } = yield* ServerConfig;
      ...
      assert.deepEqual(configState.issues, []);
      assert.isTrue(configState.keybindings.some((entry) => entry.command === "thread.previous"));
      assert.isTrue(configState.keybindings.some((entry) => entry.command === "thread.next"));
      assert.isTrue(configState.keybindings.some((entry) => entry.command === "thread.jump.3"));
    }).pipe(Effect.provide(makeKeybindingsLayer())),
  );
