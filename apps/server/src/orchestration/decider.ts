...
import {
  ...
  requireOrchestratorWorkerStatus,
  requireProject,
...
> {
  switch (command.type) {
    ...
    case "orchestrator.worker.promote": {
      ...
      // Validate foreground count < 2
      const foregroundCount = (readModel.orchestratorWorkers ?? []).filter(
        (w) => w.visibility === "foreground" && w.status !== "terminated",
      ).length;
      if (foregroundCount >= 2) {
        return yield* new OrchestrationCommandInvariantError({
          commandType: command.type,
          detail: `Cannot promote worker '${command.workerId}': already ${foregroundCount} foreground worker(s) (max 2).`,
        });
      }
      return {
        ...withEventBase({
    ...
    case "orchestrator.message.send": {
      ...
      return {
        ...withEventBase({
          aggregateKind: "orchestrator",
          aggregateId: command.toWorkerId as unknown as OrchestrationEvent["aggregateId"],
          occurredAt: command.createdAt,
          commandId: command.commandId,
        }),
    ...
    case "orchestrator.context.transfer": {
      ...
      return {
        ...
        payload: {
          ...
          transferredAt: command.createdAt,
        },
      };
    }
    ...
    case "orchestrator.dependency.set": {
      ...
      return {
        ...withEventBase({
          aggregateKind: "orchestrator",
          aggregateId: command.fromWorkerId as unknown as OrchestrationEvent["aggregateId"],
          occurredAt: command.createdAt,
          commandId: command.commandId,
        }),
        ...
        payload: {
          ...
          setAt: command.createdAt,
        },
      };
    }

    case "orchestrator.work.merge-request": {
      yield* requireOrchestratorWorker({
        readModel,
        command,
        workerId: command.workerId,
